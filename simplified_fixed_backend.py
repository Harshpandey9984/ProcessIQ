"""
Simplified fixed backend for Digital Twin Platform.
This version includes direct password comparison for debugging authentication issues.
"""
...
logging.basicConfig(level=logging.INFO, 
                    ...
                    filename='logs/simplified_fixed_backend.log',
                    filemode='a')
...
app = FastAPI(
    ...
    description="Simplified Fixed API for Digital Twin Platform",
    version="1.0.0"
)
...
# ---- Mock Data ----
# Using plain text passwords for simplified debugging
USERS_DB = {
    "admin@example.com": {
        "id": "1",
...
def authenticate_user(email: str, password: str):
    """
    Simplified authentication with direct password comparison
    """
    ...
    if password != user["password"]:
        ...
        logger.warning(f"Expected '{user['password']}', got '{password}'")
        return None
...
async def login_for_access_token(form_data: OAuth2PasswordRequestForm = Depends()):
    ...
    logger.info(f"Request form data present: {form_data}")
    
    try:
        # Log detailed request info
        logger.info(f"Login request details - username: {form_data.username}, password length: {len(form_data.password)}")
...
if __name__ == "__main__":
    import uvicorn
    logger.info("Starting simplified fixed backend server on port 8001...")
